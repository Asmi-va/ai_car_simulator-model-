...
class Network(nn.Module):
    ...
    def __init__(self, input_size, number_of_actions):
        ...
        # Each layer is fully connected to each other due to nn.Linear
...
class DeepQNetwork():
    ...
    def select_action(self, state):
        with torch.no_grad():  # Use `torch.no_grad()` instead of `volatile=True`
            probs = F.softmax(self.model(Variable(state)) * 100, dim=-1)  # Specify the dimension
        action = probs.multinomial(num_samples=1)  # Specify the number of samples
        return action.item()  # Use `.item()` to get a Python number
    ...
    def learn(self, batch_state, batch_next_state, batch_reward, batch_action):
        ...
        td_loss.backward()  # Default behavior is to retain the computation graph for backward pass
        self.optimizer.step()
